...
public:
    mem_context(mem_context* parent)
        : m_parent{parent}
    {
        if (m_parent)
    ...
    virtual void* align_alloc(::std::size_t alignment, ::std::size_t size) noexcept {
        return nullptr;
    }
...
// mono_mem_context allocates N bytes memory block, and holds it until destructor
class mono_mem_context : public mem_context {
public:
    mono_mem_context(mem_context* parent, unsigned int alignment, unsigned int capacity)
        : m_parent{parent}
        , m_storage{::std::make_unique<char[]>(m_parent->aligned_alloc(alignment, capacity))}
        , m_capacity{capacity}
        , m_occupied{0}
    {
    }
